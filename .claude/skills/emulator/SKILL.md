...
- A snapshot pins the hardware. `hw.ramSize` and the rest are baked into `snapshots/default_boot/hardware.ini` and into `ram.img`, so raising the RAM in `config.ini` changes nothing while quick boot keeps resuming that snapshot. The running instance writes what it really booted with to `hardware-qemu.ini`: when the two disagree, cold-boot it. A starved guest does not say so — it hangs a long job at startup with no port forward and no output, while short ones still pass. Confirm with `adb shell cat /proc/meminfo` before trusting a run.
- `start` defaults to `Medium_Phone`, which may no longer exist; `emulator -list-avds` says what does, and `AVD=<name>` picks it.
