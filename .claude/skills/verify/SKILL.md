---
name: verify
description: Run the format check, analyzer, and tests for this Flutter app and report only failures. Use after code changes or before committing.
---

Run all three from the repo root, even if an earlier one fails. If `flutter`/`dart` aren't on PATH, use `D:\Desktop\projects\flutter_sdk\flutter\bin\flutter.bat` / `dart.bat`.
