...
- [ ] **Attach a photo to a transaction** (schema step): a receipt taken with the camera or picked from the photo library, shown on the transaction and openable full screen, with replace and remove. The file is copied into the app's own storage, never a shared folder; picking uses the system picker so Android needs no gallery permission, while the camera needs `CAMERA` on Android and `NSCameraUsageDescription` on iOS. Neither may pull in `INTERNET` (RUN-2). Ask for product rules (`ATT-*`) before building, and settle there: how many photos per transaction, whether they are downscaled on import, whether backup and CSV export carry them (a backup with photos is no longer a small JSON), and what a restore does with a photo it hasn't got. Added 16 September 2026 at the user's request.
- [ ] **Attach a voice note to a transaction** (schema step): record in the app, play it back, replace, remove, with a length cap and something to show while recording. Needs `RECORD_AUDIO` on Android and `NSMicrophoneUsageDescription` on iOS, again with no `INTERNET` (RUN-2). No speech recognition: it would mean either unreliable on-device models across six languages or a cloud service, which the product principles rule out. It shares its storage, backup, and delete plumbing with the photo item above, so build the two together. Added 16 September 2026 at the user's request.
