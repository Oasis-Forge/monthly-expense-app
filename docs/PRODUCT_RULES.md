...
- **BUD-5** A budget repeats every period until the user changes or removes it. A change applies from the current period onward; past periods keep their limits. After the month start day moves (PER-2), a change still wins for the whole current period, even over a limit set earlier in the same calendar month.
