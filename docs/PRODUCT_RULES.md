...
- **RUN-1** After setup (RUN-3) and the walkthrough (RUN-4), an empty Home shows one clear "Add your first transaction" action. Before that first entry there is no summary card either: a card of zeros says nothing, and the welcome should have the screen.
