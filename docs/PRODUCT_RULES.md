...
- **PER-4** First day of week follows the device locale until the user changes it: its region's own convention when the app is showing the device's own language, and otherwise the convention of whichever language the user has chosen, unconnected to their phone's region. English (UK) starts the week on Monday rather than the Sunday English alone defaults to, because that's the region's own CLDR convention. Portuguese (Portugal) starts on Monday too, but that one is our own choice rather than CLDR's: CLDR and intl both carry Sunday for `pt_PT`, the same as Brazilian Portuguese, and we override it to match the convention the rest of the EU actually follows.
...
- **DEL-5** The trash holds every deleted transaction, transfer, and note (NOTE-7) — most recently deleted first, each restored by the same button (DEL-4). A deleted transfer survives the next launch: it waits out its 30 days in the database like anything else (DEL-3), and the app reads it back rather than forgetting it. A deleted recurring rule is not trash: it comes back only through the Undo its own delete offered (DEL-2), and is gone for good once that closes.
...
- **BAK-8** The phone's own automatic backup is refused for the records — the database, the attachments and the automatic backups the app keeps for undo — on both Android and iOS, only in the backup file the app makes when asked (BAK-1, BAK-6). On Android, `android:allowBackup="false"` and a `dataExtractionRules` that excludes every domain keep the database *and* the settings out of the user's Google Drive backup, so nothing the app stores at all rides along in a copy the OS makes of everything else on the phone. Android would otherwise copy them there by itself — a copy of every entry that nobody asked for, and one that would be handed to whoever next installs the app under that account. On Android 12 and up, moving to a new phone through the setup wizard's own device-to-device transfer still carries them, because that is the user taking their own data with them rather than a copy kept somewhere, and `dataExtractionRules` only governs that transfer from there. On Android 11 and below, `allowBackup="false"` turns off device transfer along with cloud backup, so a phone-to-phone move does **not** carry the records on those phones. On iOS, the app has no manifest flag to refuse backup with — instead, at startup, it marks the directories holding the database, the attachments and the automatic backups excluded from backup (`URLResourceValues.isExcludedFromBackup`, `SecurityBridge.swift`, `backup_exclusion.dart`); that exclusion does not reach `UserDefaults`, so the settings are not excluded and an iCloud or computer backup still keeps them, the same as it would for any other app. The widget's numbers are excluded as well: the app writes them to a file in a folder of the `group.com.oasisforge.monthlyexpenses` App Group container that it marks excluded the same way before the file goes in, rather than into the group's `UserDefaults`, and removes the copy that 1.31.1 and earlier kept there (`HomeWidgetBridge.swift`, WID-5). Unlike Android, iOS has no split between cloud backup and device transfer: excluding a directory this way keeps it out of an iCloud backup and a computer backup taken over cable, and Apple documents `isExcludedFromBackup` as guidance to the system rather than a guarantee that an excluded item never appears in a backup or on a restored device. What a Quick Start device-to-device migration carries is not documented anywhere authoritative either, so on iOS the backup file the app makes when asked — not the OS's own backup or migration — is the dependable way to bring the records along. A reinstall always starts empty on either platform.
...
- **INS-6** The category chart says how the period compares with the one before it. Under the total, one line: how much more or less than last month, or that it came to the same. On each category's line, the share it has risen or fallen by, beside its amount. A category with nothing in it last period is marked "new" rather than given a percentage, because there is nothing to divide by; a category that had something and now has nothing is not on the chart at all, and the line under the total is where its going shows. The earliest period on record has nothing behind it and shows neither the line nor the shares — an empty comparison is worse than none, since it reads as "you spent nothing last month". That emptiness is about the bounded span below, not merely about some record existing earlier still: a record dated before the period the bound admits is no more comparable than no record at all, and would read the same way. A period still in progress is measured against the same number of days at the start of the one before it, not the whole of it, so the 3rd of the month never reads as a huge drop from a complete previous period; a period already over is measured against the whole of the one before it instead, since there is no "so far" left for it to match against (pr56+60#4). The comparison follows the chosen account exactly as the chart does (ACC-7) and counts what the period counts, so a future-dated entry is in neither side of it (BAL-4).
...
- **WID-5** The widget refreshes after every change in the app and at midnight, when a new day can start a period or make upcoming entries count. It never uses the network, and the app shares only the numbers the widget shows, never the database. Where those numbers wait for the widget — a private file on Android, a file in the App Group container on iOS — is kept out of the phone's own backup on both, like the records (BAK-8).
...
- **NOTE-6** A due date can have a reminder at a chosen time, sent as a local notification from the device. The app asks for notification permission at the end of the walkthrough (NUDGE-7), and asks here too for anyone who was never asked there — someone updating from a version before it existed. Everything else works if it's refused. Android may deliver it a few minutes late. With app lock on, the notification says only that a note is due, and tapping it opens the note through the lock (LOCK-2). Opening the app, or any other reschedule, shortly after a reminder's time never cancels it: a time that passed within the last two hours (covering the inexact alarm's own delivery window, NUDGE-9) is left exactly as scheduled, on a note that is still open, so the pending alarm still fires. With app lock on, every such reschedule checks what the device itself currently has for it — not any memory of whether app lock just turned on, which a cold start has none of (x-reminder-lock-keep) — and rewords it once: a delivered one still sitting in the tray is replaced in place with the locked wording (without alerting again), and a still-pending one is cancelled and re-laid about a minute out with it. Once that copy itself carries the locked wording, later reschedules leave it alone, so it is not re-shown or re-laid on every cold start, language, lock or nudge change. A reschedule cancels a reminder only when the note is done, deleted, has no reminder, its time changed, or that time is more than two hours past.
...
- **ADS-17** On iOS, once the consent step (ADS-5) has allowed ads at all, the app asks iOS's own App Tracking Transparency question — whether the ads may be chosen using the device's advertising identifier — and only then starts the ad SDK, so its first request already knows the answer. The order is fixed: setup and the walkthrough (ADS-4), the consent form where the law asks for it, this question, then the SDK. It is asked only while iOS still has no answer (an earlier launch, or the consent message itself, may have settled it), only while the app is in the foreground, and never over the lock screen (LOCK-2). It is never asked where consent was refused outright, once the ads are bought away (ADS-8, PAY-1: the ad software is never started then), or on Android, Windows, Linux or macOS. Saying no still shows ads, just not personalised ones, and takes nothing else away (PAY-4); the answer can be changed later only in iOS's own Settings.
...
- **NUDGE-2** The due one is specific: it names what was due and for how much, in the words of the rule the user wrote (section 8), and it appears only while that occurrence is still unhandled. It says "due today" only when today really is the day it fell due; an occurrence carried over from an earlier, unhandled day names that day instead of claiming it is today's (rules-23-26-34#9). Several due on the same day are one notification, not one each, and names neither the amount nor the day since no single one of them is the one being announced — but it still says "were due today" only when every one of them really is; a group that carries over even one occurrence from an earlier, unhandled day says they are waiting instead, for the same reason a single one does (rules-23-26-34#9). Tapping it opens the upcoming list, and nothing is ever recorded from a notification — a proposal is confirmed by a person, exactly as a bank's is (BANK-3, ALERT-4).
...
- **NUDGE-9** Inexact alarms, so Play is never asked for the exact-alarm permission and Android may deliver a few minutes late. Reminders are rescheduled after a reboot and after a restore, as a note's are (NOTE-6). The decision is that a reminder more than two hours overdue when the phone was off should be dropped after the reboot rather than fire late, the way a reschedule from Dart already drops one (NOTE-6) — but as of 1.31.0 this is not met for the reboot path itself: `flutter_local_notifications`' own boot receiver re-arms every cached one-shot at its original time before Dart ever runs, so an overdue one still fires immediately on boot (pr59_10). Meeting it needs a receiver of the app's own ahead of the plugin's, reading and filtering its cached notifications before delegating to it — not attempted yet, since it leans on that plugin's private storage format with no device verification.
...
- **RATE-1** The app asks at most once for any version it is installed as, and only of someone who has recorded at least fifteen entries and has had it for a week. "Recorded" means typed into the add form and saved by hand: a row a recurring rule posted on its own (RCR-4), a CSV import brought in, or a backup restored is not a use of the app being counted as one, whatever the transaction list's length says. Under either line it says nothing: three entries is not an opinion, and a week is the shortest time in which the app can have been any use.
...
- **THEME-4** The colours that carry meaning are the app's own under every choice: the income and expense inks (CUR-5), the category colours (CAT-6), and the budget bar keep their hues whatever the wallpaper is. Where one carries text — the inks, the budget bar's figure — it clears 4.5:1 against the surface it sits on (A11Y-3). Where it doesn't — a category swatch, dot, or pie slice, which is never text — it clears WCAG's 3:1 non-text bar instead (pr58#9). A palette is allowed to change what the app looks like, never what a colour means.
