...
- **LANG-7** A plural message never carries an `=0` or `=1` case whose wording is anything but that number's own form. `gen_l10n` compiles an explicit case into the CLDR category of the same name, and several languages put more than the literal value in theirs — Russian's *one* holds 21, 31 and 41 — so a case written for 1 is handed out at 21 as well: "1 результат" for twenty-one of them, and a rule repeating every 21 days calling itself "Every day". A message that counts something carries its number in every case, and a wording that belongs to exactly one thing — "Every day", "tomorrow" — is a message of its own that the code reaches for before the plural. `test/l10n_test.dart` renders every counting message in all twenty-one languages at 21 and 31 and fails if either reads the way 1 does.
...
- **NUDGE-7** Permission is asked once, at the end of the walkthrough (RUN-4) and before the app's own question about the empty day (NUDGE-3). Permission first and the question second: Android grants one dialog and never another, so asking whether somebody wants a reminder the phone will not deliver spends the question on nothing. Refused, the app asks nothing further and everything else still works (NOTE-6); Settings says the phone is not allowing them and how to change that. A user who reaches a reminder another way — turning one on in Settings, or setting one on a note — is asked there instead, because they may be updating from a version that never asked at first run.
