...
- **ADS-5** Where the law asks for it (the EEA, the UK, and Switzerland), the ad network's consent form appears before the first request, and Settings keeps a "Privacy options" row to change the answer later. It never appears over the lock screen (LOCK-2): a launch that comes up locked holds the form back, and it (with everything after it — the tracking prompt (ADS-17), then the SDK) starts the moment the app is unlocked, not before. Refusing means non-personalised ads, never a nag screen or a feature withheld.
