...
- **BAK-8** The phone's own automatic backup is refused: `android:allowBackup="false"`, and `dataExtractionRules` excludes every domain from cloud backup. Android would otherwise copy the database and the settings to the user's Google Drive by itself — a copy of every entry that nobody asked for, and one that would be handed to whoever next installs the app under that account. On Android 12 and up, moving to a new phone through the setup wizard's own device-to-device transfer still carries them, because that is the user taking their own data with them rather than a copy kept somewhere, and `dataExtractionRules` only governs that transfer from there. On Android 11 and below, `allowBackup="false"` turns off device transfer along with cloud backup, so a phone-to-phone move does **not** carry the records on those phones, and the app's backup reminder says so; the way to bring them along either way is the backup file the app makes when asked (BAK-1, BAK-6). A reinstall always starts empty.
