...
- **BAK-8** The phone's own automatic backup is refused: `android:allowBackup="false"`, and `dataExtractionRules` excludes every domain from cloud backup. Android would otherwise copy the database and the settings to the user's Google Drive by itself — a copy of every entry that nobody asked for, and one that would be handed to whoever next installs the app under that account. On Android 12 and up, moving to a new phone through the setup wizard's own device-to-device transfer still carries them, because that is the user taking their own data with them rather than a copy kept somewhere, and `dataExtractionRules` only governs that transfer from there. On Android 11 and below, `allowBackup="false"` turns off device transfer along with cloud backup, so a phone-to-phone move does **not** carry the records on those phones; the way to bring them along either way is the backup file the app makes when asked (BAK-1, BAK-6). A reinstall always starts empty.
...
- **NUDGE-5** It goes quiet by itself: three in a row that are neither opened nor followed by an entry that day, and it stops, with Settings saying why and one tap to start it again. A reminder people swipe away has already stopped working, and it is better for it to admit that than to keep teaching them that the app is worth ignoring. A stretch spent with the phone itself blocking notifications (NUDGE-7) is never counted towards those three: it was never delivered to be ignored, and blaming the person for the phone's own setting would be the wrong lesson. Nor is one the phone dropped at a restart rather than show it late or in the quiet hours (NUDGE-9): nobody saw it either, so the boot receiver notes when each was due and the app leaves those days out of the count, though an entry that day still answers.
- **NUDGE-6** At most one a day, whatever is waiting, and never between ten at night and eight in the morning by the device's own clock. That holds after a restart too: a nudge whose time passed while the phone was off is dropped rather than sent if the phone comes back in those hours (NUDGE-9). A day with something due is not also told that it is empty.
...
- **NUDGE-9** Inexact alarms, so Play is never asked for the exact-alarm permission and Android may deliver a few minutes late. Reminders are rescheduled after a reboot and after a restore, as a note's are (NOTE-6). A reboot follows the same two-hour rule as a reschedule from Dart (NOTE-6): a reminder whose time passed while the phone was off by less than two hours still arrives as soon as the phone is back, and one more than two hours overdue is dropped rather than fire late. One of the app's own nudges is dropped too when the phone comes back inside the quiet hours, however little it is overdue, so a nudge never arrives at night because the phone was off at its time (NUDGE-6), and a dropped empty-day nudge is never counted as ignored (NUDGE-5); a note's reminder has no quiet hours, its time being the user's own choice. An app update, which lays the reminders again the same way, follows it too; and because an update keeps the alarms a restart clears, the receiver also disarms the alarm of each reminder it drops, since one the phone held back past its time (Doze, a seldom-opened app's standby bucket) would otherwise still fire late. `flutter_local_notifications`' own boot receiver would re-arm every cached one-shot at its original time before Dart ever runs, so an overdue one fired immediately on boot (pr59_10); the app declares a receiver of its own in its place, which drops the one-shots more than two hours overdue from the plugin's cache and then hands the rest to the plugin to lay again. The two hours are one number: `passedReminderGrace` in Dart, and a test holds the receiver's equal to it. A reminder that repeats is left alone, and a cache the receiver cannot read is laid again whole, so a reminder still due is never lost to the rule. The receiver reads that plugin's private storage, so a test pins the plugin version it was checked against and fails on an upgrade until the format is looked at again.
