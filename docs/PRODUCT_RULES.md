...
- **BUD-2** Spent = the category's expenses in the period (BAL-4 applies). Remaining = limit − spent. A range that isn't one whole period — a custom range, a whole year, part of a period — has no single limit to price it against, so nothing showing spending over such a range (the PDF report, PDF-2) attaches a budget figure to it.
