...
- Release signing reads `android/key.properties` (gitignored); without it, a release build fails fast unless `-PallowDebugSigning=true` (or `CI=true`, set by GitHub Actions) is passed, in which case it falls back to debug signing for a local test build.
