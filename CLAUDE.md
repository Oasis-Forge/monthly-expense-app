...
- Every merged PR is a release: bump the version on the branch with `/release [major|minor|patch]` (SemVer `x.y.z+N` plus a `CHANGELOG.md` entry; CI checks it), and the merge tags `vX.Y.Z`, builds the APK and the App Bundle, attaches the APK to a draft GitHub Release and keeps both in the run's artifacts. `/release` also writes Play's release notes in every listing language to `store/play/release-notes/X.Y.Z.txt`. It builds locally into `dist/` (gitignored) only with `--build`, for hand-testing a release build or uploading to Play before the merge — two release builds take about twelve minutes, so ask for it when it is wanted, and rebuild after any later app change on the branch.
