...
Money? evaluateAmount(String input, {int maxDecimals = 3}) {
  ...
  var total = Money.tryParse(parts.first, maxDecimals: maxDecimals);
  for (var i = 1; i < parts.length && total != null; i += 2) {
    final operand = Money.tryParse(parts[i + 1], maxDecimals: maxDecimals);
